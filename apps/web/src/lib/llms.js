...
export async function llmsTxt(opts = {}) {
  ...
  const lines = [
    ...
    `- \`${base}/topics/{keyword}.md\` — the same river as Markdown, to read rather than to poll`,
    ...
    '',
    '## Any listing, as a feed',
    '',
    'Every page that lists things publishes its own document at its own address,',
    'in four formats — `.rss`, `.atom`, `.json` and `.md`. Same page, plus an',
    'extension:',
    '',
    `- \`${base}/{slug}.rss\` — one feed of the directory: our copy of what it published`,
    `- \`${base}/authors/{slug}.rss\` — one person, across every feed of theirs we know about`,
    `- \`${base}/feed.rss\` — the directory itself: feeds as they are added to it`,
    `- \`${base}/blogs.rss\` — the same for one category (.news, .podcasts, .music, .videos, .comics, .lives, .reels)`,
    `- \`${base}/search.rss?q=lisp\` — a standing search, which is the closest thing here to an alert`,
    '',
    'The `.md` spelling is the one worth knowing about if you are not a feed',
    'reader: it is the same river written as prose — a heading per item, the date',
    'and byline under it, the summary below that — so a document can be read',
    'without an XML parser or a JSON schema. Nothing polls it; it is for reading.',
    '',
    'Fifty items by default, up to 200 with ?limit=. A feed page also serves .m3u',
    'and .pls where its entries are playable. These documents are our copy rather',
    'than a proxy: they carry what the last crawl stored, with summaries reduced',
    'to plain text and links that come back to the reader here. The publisher’s',
    'own feed URL is on the page and in /api/feeds/{slug} if you want the source.',
    '',
    'A sponsored item appears once every ten entries, at most three per document,',
    'never as the last entry. Each is marked: `<category>Sponsored</category>` in',
    'RSS, a `Sponsored` tag plus `_crawlproof` in JSON Feed, and a bold Sponsored',
    'line under the heading in Markdown. Filter on those if you are indexing.',
    '',
    '## MCP server',
    '',
    ...
    '- Each author has a page at /authors/{slug} carrying schema.org Person with sameAs, and a <link rel="openprofile"> to /authors/{slug}/openprofile.md.',
